...
"4485 units each with 2961 hit points (immune to radiation; weak to fire, cold) with an attack that does 12 slashing damage at initiative 4"
        ...
        public void WinningArmyUnitsBoost(string[] input, int boost, int expected)
        {
            Program.Parse(input);
            Program.Boost(boost);
            Assert.That(Program.RunFight(), Is.EqualTo(0));
            Assert.That(Program.WinningArmyUnits(), Is.EqualTo(expected));
        }
...
"801 units each with 4706 hit points (weak to radiation) with an attack that does 116 bludgeoning damage at initiative 1",
"4485 units each with 2961 hit points (immune to radiation; weak to fire, cold) with an attack that does 12 slashing damage at initiative 4"
        }, 1570, TestName = "SmallestBoost = 1570")]
        public void SmallestBoost(string[] input, int expected)
        {
            Program.Parse(input);
            Assert.That(Program.SmallestBoost(), Is.EqualTo(expected));
        }
    }
}
