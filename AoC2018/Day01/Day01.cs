...
namespace Day01
{
    class Program
    {
        const long MAX_NUM_CYCLES = 256;
        const long MAX_NUM_FREQUENCIES = 1L * 1024 * 512;
        const int MAX_NUM_FREQUENCY_CHANGES = 2048;
        static int sNumFrequencyChanges;
        readonly static long[] sFrequencyChanges = new long[MAX_NUM_FREQUENCY_CHANGES];
        readonly static byte[] sFrequencies = new byte[MAX_NUM_FREQUENCIES];
        ...
        {
            if (lines.Length > MAX_NUM_FREQUENCY_CHANGES)
            {
                throw new InvalidProgramException($"Too many frequency changes {lines.Length} Max:{MAX_NUM_FREQUENCY_CHANGES}");
            }

            sNumFrequencyChanges = 0;

            foreach (var line in lines)
            {
                var value = long.Parse(line.Trim());
                sFrequencyChanges[sNumFrequencyChanges] = value;
                ++sNumFrequencyChanges;
            }
        }

        public static long FindFinalFrequency()
        {
            var frequency = 0L;
            for (var i = 0; i < sNumFrequencyChanges; ++i)
            {
                var delta = sFrequencyChanges[i];
                frequency += delta;
            }
            return frequency;
        }

        public static long FindFirstRepeatFrequency()
        {
            for (var i = 0; i < MAX_NUM_FREQUENCIES; ++i)
            {
                sFrequencies[i] = 0;
            }

            var frequency = 0L;
            for (var r = 0; r < MAX_NUM_CYCLES; ++r)
            {
                for (var i = 0; i < sNumFrequencyChanges; ++i)
                {
                    var index = frequency + MAX_NUM_FREQUENCIES / 2;
                    if (sFrequencies[index] == 1)
                    {
                        return frequency;
                    }
                    sFrequencies[index] = 1;
                    var delta = sFrequencyChanges[i];
                    frequency += delta;
                }
            }
            throw new InvalidProgramException($"No repeat found after {MAX_NUM_CYCLES} cycles");
        }
